...
Currently, the most commonly used ingress controller is [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/). This works well for many people, and _**Node.js Ingress Controller**_ does not seek to replace it. Instead, our goals are as follows:
...
If all goes well, the following steps should happen automatically over the next few minutes. _**You shouldn't need to do anything while this happens,**_ but it may help to understand the process in case you need to troubleshoot.
...
3. Your external-dns instance (if applicable) will see this, and start pointing DNS records at this IP address. (_**If you're not using external-dns, you'll have to do this step manually.**_) Now people on the public internet will be able to reach the ingress controller via the relevant hostnames — but your sites are not quite ready yet.
4. Your HTTPS `Ingress`es (i.e. those that have a `tls` section) need to have certificates provisioned. Assuming you're using cert-manager, it will do this for you as follows:
